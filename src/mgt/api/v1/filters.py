...
from utils.data_cleaners import remove_non_digits
...
class CustomerFilter(filters.FilterSet):
    ...
    cpf = filters.CharFilter(lookup_expr='icontains', method='clean_and_filter_cpf')
    phone = filters.CharFilter(lookup_expr='icontains', method='clean_and_filter_phone')
    ...
    def clean_and_filter_cpf(
        self, queryset: QuerySet[Customer], name: str, value: str
    ) -> QuerySet[Customer]:
        """
        Cleans the CPF value by removing non-digit characters before filtering.
        """
        if value:
            cleaned_value = remove_non_digits(value=value)
            return queryset.filter(cpf__icontains=cleaned_value)
        return queryset

    def clean_and_filter_phone(
        self, queryset: QuerySet[Customer], name: str, value: str
    ) -> QuerySet[Customer]:
        """
        Cleans the phone value by removing non-digit characters before filtering.
        """
        if value:
            cleaned_value = remove_non_digits(value=value)
            return queryset.filter(phone__icontains=cleaned_value)
        return queryset
